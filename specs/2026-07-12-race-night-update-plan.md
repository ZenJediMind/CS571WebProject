...
- [ ] **Step 2: Write `tests/model.test.mjs`** — port of the existing scratch fixture using `node:test`:
...
- [ ] **Step 3: Write `tests/engine.test.mjs`** — autopilot completion + determinism (controller inline for now; Task 4 moves it into `src/game/autopilot.js`):
...
- [ ] **Step 4: Add the npm script** — in `package.json` scripts: `"test": "node --test tests/"`
...
- [ ] **Step 1: Replace `stepRace` in `src/game/engine.js`** (keep `MAX_STEP_SECONDS = 0.05`; add the new frame cap next to it):
...
- [ ] **Step 2: Append regression tests to `tests/engine.test.mjs`**
...
- [ ] **Step 3: Verify** — `npm test` (existing lap/determinism tests still pass — 1/60s steps are below the substep cap, so behavior is unchanged for normal frames) + `npm run lint` + `npm run build`.
...
- [ ] **Step 1: `storage.js`** — make `writeKey` report the outcome (and correct the misleading comment):
...
- [ ] **Step 2: `courseService.saveCourse`** — return `null` on a failed write:
...
- [ ] **Step 3: `carService.savePlayerCar`** — `return writeKey(CAR_KEY, { imageDataUrl })`.

- [ ] **Step 4: `CourseBuilder.jsx`** — add `const [saveError, setSaveError] = useState(false)`; guard the save paths so a failed save stays on the page:
...
- [ ] **Step 5: `CarDesigner.jsx`** — same pattern: `saveError` state, `handleSave` only navigates when `savePlayerCar(paint.current)` returns true, matching dismissible Alert ("Your drawing is still here…").

- [ ] **Step 6: Write `tests/storage.test.mjs`** — Node has no `localStorage`, which exercises the failure path directly (`storage.js` has no internal imports, so it loads as-is):
...
- [ ] **Step 7: Verify** — `npm test && npm run lint && npm run build`; manual smoke: saving a course and a car still works and navigates; DevTools → block storage (or fill quota) → save shows the alert and stays on the page.
...
- [ ] **Step 1: Write `src/game/autopilot.js`**
...
- [ ] **Step 2: Refactor `tests/engine.test.mjs`** — delete `driveInputs`, `center`, `normalizeAngle`; instead:
...
- [ ] **Step 3: Verify** — `npm test` all pass; `npm run lint`; `npm run build`.
...
- [ ] **Step 1: Write `src/services/settingsService.js`**
...
- [ ] **Step 2: Write `src/pages/Settings.jsx`**
...
- [ ] **Step 3: Wire route and menu** — `App.jsx`: `import Settings from './pages/Settings'` and add `<Route path="/settings" element={<Settings />} />` inside the layout route. `Home.jsx`: append `{ label: 'Settings', to: '/settings' }` to `SECONDARY_LINKS`.

- [ ] **Step 4: Verify** — `npm run lint && npm run build`; smoke in `npm run dev`: toggle options, reload page, options persist.
...
- [ ] **Step 1: courseModel** — add to `PIECES`: `OIL: 'oil', RAMP: 'ramp'`; add to `BASE_OPENINGS`: `[PIECES.OIL]: ['N', 'S'], [PIECES.RAMP]: ['N', 'S'],`. (`isDrivable` already returns true for any non-obstacle piece.)

- [ ] **Step 2: render.js piece art** — inside `drawTrackPiece` after the PIT block, add:
...
- [ ] **Step 3: Builder palette** — in `PALETTE_SECTIONS` Specials items, append:
...
- [ ] **Step 4: Mad Town GP showcase** — in `templates.js` `MAD_TOWN_CELLS`: top row straights become `[2, 3, 4, 6, 8, 9]` plus `{ row: 1, col: 7, piece: PIECES.RAMP, rotation: 90 }`; bottom row straights become `[9, 8, 6, 4, 3, 2]` plus `{ row: 7, col: 5, piece: PIECES.OIL, rotation: 90 }` (keep the existing BOOST at row 7 col 7 and START at row 1 col 5).

- [ ] **Step 5: Test additions** to `tests/model.test.mjs`:
...
- [ ] **Step 6: Verify + commit** — `npm test && npm run lint && npm run build`; smoke: new pieces appear in the palette, place/rotate/validate a loop through them.
...
- [ ] **Step 1: New constants** (near the existing ones):
...
- [ ] **Step 2: `createRaceState(course, options = {})`** — accept `{ maxSpeedFactor = 1 }`; add to the returned object:
...
- [ ] **Step 3: Replace `applyThrottle` and `applySteering`**
...
- [ ] **Step 6: Write `tests/engine-features.test.mjs`**
...
- [ ] **Step 7: Verify** — `npm test` (all files) + `npm run lint` + `npm run build`. Manual smoke: Space drifts audibly tighter on `/#/race/tpl-ring-road`; oil/ramp behave on Mad Town.
...
- [ ] **Step 1: Write `src/game/ghosts.js`**
...
- [ ] **Step 2: Write `src/services/ghostService.js`**
...
- [ ] **Step 3: Write `tests/ghosts.test.mjs`**
...
- [ ] **Step 4: Verify + commit** — `npm test && npm run lint && npm run build`.
...
- [ ] **Step 1: Write `src/game/audio.js`**
...
- [ ] **Step 2: Verify + commit** — `npm run lint && npm run build` (no Node test — WebAudio is browser-only; Task 11 smoke-tests it).
...
- [ ] **Step 1: Replace the car/frame section at the bottom of `render.js`** (everything from `const CAR_DRAW_SIZE` down) with:
...
- [ ] **Step 2: Verify** — `npm run lint && npm run build` (call-site update happens in Task 11; the build will fail if `useRaceLoop` still uses the old signature — update the call there in the same commit if needed, or commit Tasks 10+11 together). **Preferred: implement Task 10 and Task 11 back-to-back, verifying the build after Task 11, with a single combined smoke test.**
...
- [ ] **Step 1: Rewrite `src/hooks/useRaceLoop.js`**
...
- [ ] **Step 2: Update `src/pages/Race.jsx`** — changes only (rest of the file stays):
...
- [ ] **Step 3: Split chip styles** in `theme.css`:
...
- [ ] **Step 4: Verify** — `npm test && npm run lint && npm run build`. Manual smoke on `/#/race/tpl-ring-road`:
...
- [ ] **Step 2:** Full manual regression in `npm run dev`: every navigation edge still works; Builder places/rotates/validates oil + ramp; Browse thumbnails render the new pieces; Results/Leaderboard unaffected; Settings persists across reloads; clear site data → templates and defaults still load.
- [ ] **Step 2b (P0 verification):** DevTools → Performance → 6× CPU throttle → race a lap: the HUD timer tracks wall-clock time (compare against a stopwatch). DevTools → Application → block storage (or fill quota): course and car saves show the error alert and stay on the page.
- [ ] **Step 3:** Accessibility spot-check: Settings radios/switch labeled and keyboard-reachable; split chip readable by −/+ prefix; sound never plays before a keypress.
- [ ] **Step 4:** Commit any tuning deltas, then `git push origin dev`. Do NOT merge to `main`.
